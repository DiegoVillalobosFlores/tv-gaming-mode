...
install -m 755 bin/tv-mode.sh bin/apollo-display.sh bin/tv-mode-watch.sh \
               bin/tv-mode-boot.sh "$BIN/"
# Sourced by the two watchers, not run on its own, so it is not executable.
...
# the installer's.
install -m 644 systemd/tv-mode-watch.service systemd/tv-mode-boot.service "$UNITS/"
